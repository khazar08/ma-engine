...
@st.cache_data(show_spinner=False)
def cached_backtest():
    result = backtest_screener(load_universe(), cfg=DEFAULT_CONFIG)
    rows = [{"acquirer": a, "actual target": t, "engine rank": (r if r else "unranked")}
            for a, t, r in result.ranks]
    return result.summary_line(), pd.DataFrame(rows)
...
if not run:
    ...
    with st.expander("How the screener validates — hit-rate@K"):
        summary_line, ranks_df = cached_backtest()
        st.write(summary_line)
        st.dataframe(ranks_df, width="stretch", hide_index=True)
    st.stop()
